...
/** Same MapTiler style as the regular home map (pickup preview only). */
const DRIVER_PICKUP_MAP_STYLE =
...
export default function DriverRideScreen() {
  ...
  const runRequestAction = async (
    requestId: number,
    action: "accept" | "reject" | "cancel" | "start" | "arrived"
  ) => {
    if (!driverUserId) return;
    ...
    try {
      ...
      if (action === "start") await startDrivingToCustomer(requestId, driverUserId);
      if (action === "arrived") await markRideArrived(requestId, driverUserId);
      ...
      return (
        <View style={styles.actionColumn}>
          ...
          <TouchableOpacity
            ...
            onPress={() => runRequestAction(item.id, "arrived")}
            disabled={isBusy}
          >
            <Text style={styles.actionButtonText}>{t("ride_mark_arrived")}</Text>
          </TouchableOpacity>
        </View>
      );
    }
  ...
  const renderItem = ({ item }: { item: DriverRideRequest }) => {
    ...
    const showPickupMap =
      ...
      item.status === "driving_to_customer";
    const showExpandedMapCta =
      item.status === "accepted" ||
      item.status === "on_the_way" ||
      item.status === "driving_to_customer" ||
      item.status === "arrived";
    ...
    <View style={styles.card}>
      ...
      {phaseKey ? <Text style={styles.trackingPhase}>{t(phaseKey)}</Text> : null}
      {showPickupMap ? (
        <View style={styles.pickupMapWrap}>
          <MapView
            style={styles.pickupMap}
            mapStyle={DRIVER_PICKUP_MAP_STYLE}
            scrollEnabled={false}
            rotateEnabled={false}
            pitchEnabled={false}
            logoEnabled={false}
          ...
          >
            <Camera
              defaultSettings={{
                centerCoordinate: [item.pickup_lon, item.pickup_lat],
                zoomLevel: 14,
              }}
              ...
              animationMode="flyTo"
            />
            <PointAnnotation id={`pickup_${item.id}`} coordinate={[item.pickup_lon, item.pickup_lat]}>
              <View style={styles.pickupMapMarker}>
                <Ionicons name="navigate" size={18} color="#fff" />
              </View>
            </PointAnnotation>
          </MapView>
          <Text style={styles.pickupMapHint}>{t("ride_pickup_map_hint")}</Text>
        </View>
      ) : null}
      {showExpandedMapCta ? (
        <TouchableOpacity
        ...
        >
          ...
          <Text style={styles.openMapButtonText}>{t("ride_tracking_open_map_driver")}</Text>
        </TouchableOpacity>
      ) : null}
...
const styles = StyleSheet.create({
  ...
  pickupMap: { width: "100%", height: 160 },
  pickupMapMarker: {
    width: 36,
    height: 36,
    borderRadius: 18,
  ...
  pickupMapHint: { fontSize: 11, color: "#555", marginTop: 6, textAlign: "center" },
  openMapButton: {
    flexDirection: "row",
    alignItems: "center",
    justifyContent: "center",
