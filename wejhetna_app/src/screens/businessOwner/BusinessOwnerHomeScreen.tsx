...
export default function BusinessOwnerHomeScreen({ navigation }: Props) {
  ...
  const getRoute = async () => {
    ...
    try {
      ...
      if (code === 1) {
        Alert.alert(
          t("location_required_title") || title,
          t("location_required_for_navigation") ||
            "Please enable location access to start navigation."
        );
      } else if (isLocationTimeoutOrUnavailableError(error)) {
        Alert.alert(
          t("route_location_timeout_title") ||
            (currentLanguage === "he" ? "לא הצלחנו לקבל מיקום" : "Could not get location"),
          t("route_location_timeout_body") ||
            (currentLanguage === "ar"
              ? "GPS يستغرق وقتاً طويلاً أو الإشارة ضعيفة. انتقل لمكان مفتوح وحاول مرة أخرى."
              : currentLanguage === "he"
                ? "ה-GPS איטי או שאין קליטה. נסי במקום פתוח, המתיני כמה שניות ונסי שוב."
                : "GPS is taking too long or signal is weak. Move to an open area and try again.")
        );
      } else {
        const message =
          error?.message ||
