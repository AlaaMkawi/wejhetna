"""
Seed Laqiya places from OSM CSV into the database.
...
Usage:
    python seed_laqiya_places.py --preview
...
CSV_PATH = Path(__file__).resolve().parent / "data" / "osm_places_laqiya.csv"
PREVIEW_CSV_PATH = Path(__file__).resolve().parent / "data" / "osm_places_laqiya_preview.csv"
...
def _normalize_name(text: str) -> str:
    ...
    for prefix in ("مسجد", "مدرسة", "مطعم", "دكان", "مركز", "ملعب", "حديقة", "سوق", "مكتب"):
...
def _is_clinic(row: OsmRow) -> bool:
    return row.healthcare in CLINIC_HEALTHCARE_TAGS or row.amenity in CLINIC_AMENITY_TAGS
...
def _build_descriptions(row: OsmRow) -> tuple[str, str, str, str]:
    ...
    desc_ar = f"{ar_type} في {CITY_NAME_AR} — {row.name_ar}."
    desc_he = f"{he_type} ב{CITY_NAME_HE} — {row.name_he}."
    desc_en = f"{en_type} in {CITY_NAME_EN} — {_english_name(row.name_ar, row.name_he, row.name)}."

    if row.address:
        desc_ar += f" العنوان: {row.address}."
        desc_he += f" כתובת: {row.address}."
        desc_en += f" Address: {row.address}."
...
def _arabic_to_hebrew_fallback(name_ar: str) -> str:
    ...
    return result if _is_hebrew(result) else name_ar
...
def _build_address(street: str, housenumber: str) -> str:
    ...
    if street and housenumber:
        return f"{street} {housenumber}, {CITY_NAME_EN}"
    if street:
        return f"{street}, {CITY_NAME_EN}"
    return ""
...
def _load_csv_rows() -> list[OsmRow]:
    ...
    with CSV_PATH.open(encoding="utf-8", newline="") as f:
        ...
        for line in reader:
            ...
            row.opening_hours = _parse_osm_opening_hours(row.opening_hours_raw) or _default_hours(cat_id)
...
def preview_laqiya_places() -> None:
    ...
    print("Laqiya OSM places — preview (no database changes)")
    print(f"Preview saved to: {PREVIEW_CSV_PATH}")
...
def _place_exists(db, row: OsmRow) -> bool:
    ...
    places = (
        ...
        .filter(Place.city_id == LAQIYA_CITY_ID)
        .all()
    )
    ...
    for place, loc in places:
        ...
        if _names_similar(row.name_ar, place.name_ar or "") or _names_similar(row.name_he, place.name_he or ""):
            return True
...
def seed_laqiya_places() -> None:
    ...
    try:
        city = db.query(City).filter(City.id == LAQIYA_CITY_ID).first()
        if not city:
            raise RuntimeError(f"City id={LAQIYA_CITY_ID} (Laqiya) not found in database.")
    ...
    print("Laqiya OSM places seed finished.")
